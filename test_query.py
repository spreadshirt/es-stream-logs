...
class QueryTest(unittest.TestCase):
    ...
    def setUp(self):
        self.config = Config(default_endpoint='default', endpoints=[], indices=[],
                             field_format={}, default_fields={}, queries=[])
    ...
    def test_as_url(self):
        ...
        query = Query(self.config)
        self.assertEqual(query.as_url('/'), '/?dc=default&index=application-%2A&from=now-15m&to=now&interval=auto')

        query = Query(self.config, level='WARN')
        self.assertEqual(query.as_url('/'), '/?dc=default&index=application-%2A&from=now-15m&to=now&interval=auto&level=WARN')
    ...
    def assert_defaults(self, query, args=None):
        ...
        self.assertEqual(query.from_timestamp, 'now-15m')
